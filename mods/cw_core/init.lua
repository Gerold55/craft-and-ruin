...
local biome_tint = dofile(minetest.get_modpath("cw_core").."/biome_tint.lua").biome_tint or cw_core.biome_tint

local MP = minetest.get_modpath("cw_core")
...
local function include(file)
  ...
  if not ok then
    minetest.log("error", "[cw_core] failed to load "..file..": "..dump(err))
  else
    minetest.log("action", "[cw_core] loaded "..file)
  end
end
...
minetest.register_on_newplayer(function(player)
    ...
    for i = 1,200 do
        ...
        local y = minetest.get_spawn_level(x, z) or 5
        local pos = {x=x, y=y+2, z=z}

        local below = minetest.get_node({x=x,y=y,z=z}).name
